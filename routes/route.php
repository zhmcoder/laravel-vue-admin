...
Route::group([
    ...
    $router->group(['middleware' => config('admin.route.middleware')], function (Router $router) {
        ...
        $router->post('admin_deep_upload_image_', 'HandleController@uploadImage')->name('admin.handle-upload-image');
        $router->post('admin_deep_upload_file_', 'HandleController@uploadFile')->name('admin.handle-upload-file');
        $router->post('admin_deep_handle_form_', 'HandleController@handleForm')->name('admin.handle-form');
        $router->post('admin_deep_handle_action_', 'HandleController@handleAction')->name('admin.handle-action');
    });
});
